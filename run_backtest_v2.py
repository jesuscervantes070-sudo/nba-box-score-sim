"""Runner for FROZEN HISTORICAL BACKTEST RE-EVALUATION V2 -- re-scores the EXACT, UNCHANGED
BACKTEST_V1_HOLDOUT (same 83 games, same N_SIMULATIONS=250, same seed formula/MODEL_VERSION, same
metric definitions, same baselines) after CURRENT-SEASON DEFENSE + ROLE REFRESH V1's truth
intervention. Writes separate backtest_v2_games.json / backtest_v2_summary.json -- NEVER
overwrites the V1 files. Run directly: python3 run_backtest_v2.py
"""
import hashlib
import json
import time
...
INTERVENTION_VERSION = "defense-role-refresh-v1"  # this phase's own intervention label, NOT a
# change to MODEL_VERSION/seed formula -- V2 deliberately reuses the EXACT SAME seed sequence as
...
def verify_holdout_integrity():
    """Hard assertion, not a soft check -- refuses to run V2 at all if the holdout file was ever
...
def run_mode(game_ids, mode, label):
    print(f"\n{'='*70}\nV2: Running {label} ({mode}) over {len(game_ids)} games, n_sims={hpb.N_SIMULATIONS}\n{'='*70}")
    t0 = time.time()
...
def _run_one_mode_isolated(mode_label: str):
    ...
    phase runs them as two separate `python3 run_backtest_v2.py pregame` / `... oracle` invocations
    to test whether isolating them avoids it, per this phase's own explicit instruction)."""
    ...
    print(f"Cleared all estimator/snapshot/reference caches before V2 {mode_label} evaluation.")

    mode = hgs.MODE_PREGAME_EXPECTED if mode_label == "pregame" else hgs.MODE_ORACLE_PARTICIPANTS
    result = run_mode(game_ids, mode, f"{mode_label.upper()} (V2, isolated process)")
    ...
    out_path = BACKTESTS_DIR / f"_v2_partial_{mode_label}.json"
    with open(out_path, "w") as f:
        json.dump(payload, f, indent=2, sort_keys=True, default=str)
    print(f"\nWrote intermediate result: {out_path}")
    _, checksum_after = verify_holdout_integrity()
    assert checksum_after == holdout_checksum, "HOLDOUT FILE WAS MODIFIED DURING V2 RUN -- investigate immediately"
...
def merge_partials():
    ...
    into the final backtest_v2_games.json / backtest_v2_summary.json -- same output shape as the
    single-process `main()` path below would have produced."""
    with open(BACKTESTS_DIR / "_v2_partial_pregame.json") as f:
        pregame_partial = json.load(f)
    with open(BACKTESTS_DIR / "_v2_partial_oracle.json") as f:
    ...
    with open(BACKTESTS_DIR / "backtest_v2_games.json", "w") as f:
    ...
    with open(BACKTESTS_DIR / "backtest_v2_summary.json", "w") as f:
        json.dump(summary, f, indent=2, sort_keys=True, default=str)
    print("Merged partial results -> backtest_v2_games.json / backtest_v2_summary.json")
...
def main():
    ...
    # Explicit, documented cache clear -- no stale V1-truth profiles survive into V2.
    hpb.clear_backtest_caches()
    print("Cleared all estimator/snapshot/reference caches before V2 evaluation.")

    pregame_result = run_mode(game_ids, hgs.MODE_PREGAME_EXPECTED, "PREGAME_EXPECTED (V2)")
    oracle_result = run_mode(game_ids, hgs.MODE_ORACLE_PARTICIPANTS, "ORACLE_PARTICIPANTS (V2)")
    ...
    print("\n\n=== WROTE backtests/backtest_v2_games.json and backtests/backtest_v2_summary.json ===")
    print(json.dumps({"pregame_metrics": pregame_metrics, "oracle_metrics": oracle_metrics,
                       "baseline_metrics": baseline_metrics, "delta": delta}, indent=2, default=str))
